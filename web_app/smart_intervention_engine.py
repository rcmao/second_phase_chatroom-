...
class SmartInterventionEngine:
    ...
    def __init__(self):
        ...
        # === 行为检测参数 ===
        self.silence_threshold = int(os.getenv('SILENCE_THRESHOLD', '90'))  # 个人沉默 60s → 90s
        self.global_cooldown_seconds = int(os.getenv('GLOBAL_COOLDOWN', '30'))
        self.agenda_transition_threshold = int(os.getenv('AGENDA_TRANSITION_THRESHOLD', '60'))  # 群体沉默 30s → 60s
        ...
        # === 活跃讨论保护 ===
        self.active_discussion_window_seconds = 60
        self.active_discussion_min_unique_users = 2
        self.active_discussion_min_msg_per_minute = 0.5
    ...
    def _is_admin_user(self, user_id: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                print(f"⚠️ [管理员检查] 数据库查询失败: {e}")
            
            # 如果数据库查询失败，使用已知的管理员ID列表作为后备
            admin_user_ids = {'1', '5', '6', '8', '9', '10'}  # 管理员用户ID列表
